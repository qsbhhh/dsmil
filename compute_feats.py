    ...
    with torch.no_grad():
        for i in range(0, num_bags):
            ...
            for idx, low_patch in enumerate(low_patches):
                # feats_tree_list.extend([feats_list[idx]])
                # xx=low_patch.split('/')[-1]
                # low_patch_location.append(low_patch.split('/')[-1])
                ...
                if len(high_patches) == 0:
                    ...
                    for high_patch in high_patches:
                        ...
                        if fusion == 'fusion':
                            feats = 1 * feats.cpu().numpy() + 0 * feats_list[idx]#x10 x5
                        if fusion == 'cat':
                            feats = np.concatenate((feats.cpu().numpy(), 0.25 * feats_list[idx]), axis=-1)
                        feats_tree_list.extend(feats)

                #         top_patchs = glob.glob(high_patch.replace('.jpeg', os.sep + '*.jpeg'))
                #         if len(top_patchs) == 0:
                #             pass
                #         else:
                #             for top_patch in top_patchs:
                #                 t_img = Image.open(top_patch)
                #                 t_img = VF.to_tensor(t_img).float().cuda()
                #                 t_feats, t_classes = embedder_high(t_img[None, :])
                #                 if fusion == 'fusion':
                #                     t_feats = t_feats.cpu().numpy() + feats
                #                 feats_tree_list.extend(t_feats)
                #                 xx=top_patch.split('/')
                #                 ss=xx[-3]+'/'+xx[-2]+'/'+xx[-1]
                #                 # top_patch_location.append(ss)
                #         feats_tree_list.extend(feats)
                        xx=high_patch.split('/')
                        ss=xx[-2]+'/'+xx[-1]
                        high_patch_location.append(ss)
            ...
            if len(feats_tree_list) == 0:
                ...
                # df1=pd.DataFrame(top_patch_location)
                df2=pd.DataFrame(high_patch_location)
                # df3=pd.DataFrame(low_patch_location)
                os.makedirs(os.path.join(save_path, bags_list[i].split(os.path.sep)[-2]), exist_ok=True)
                # os.makedirs(os.path.join(save_path, bags_list[i].split(os.path.sep)[-2],'top_location'), exist_ok=True)
                os.makedirs(os.path.join(save_path, bags_list[i].split(os.path.sep)[-2], 'high_location'), exist_ok=True)
                # os.makedirs(os.path.join(save_path, bags_list[i].split(os.path.sep)[-2], 'low_location'),exist_ok=True)

                df.to_csv(os.path.join(save_path, bags_list[i].split(os.path.sep)[-2],
                                       bags_list[i].split(os.path.sep)[-1] + '.csv'), index=False, float_format='%.4f')
                # df1.to_csv(os.path.join(save_path, bags_list[i].split(os.path.sep)[-2],'top_location',
                #                        bags_list[i].split(os.path.sep)[-1] + '.csv'), index=False, float_format='%.4f')
                df2.to_csv(os.path.join(save_path, bags_list[i].split(os.path.sep)[-2],'high_location',
                                        bags_list[i].split(os.path.sep)[-1] + '.csv'), index=False, float_format='%.4f')
                # df3.to_csv(os.path.join(save_path, bags_list[i].split(os.path.sep)[-2], 'low_location',
                #                                                  bags_list[i].split(os.path.sep)[-1] + '.csv'), index=False, float_format='%.4f')
            print('\n')

# python3.6 compute_feats.py --weights_high=weights_high --weights_low=weights_low
def main():
    ...
    parser.add_argument('--magnification', default='tree', type=str,
    ...
    parser.add_argument('--weights_high', default='weights_high', type=str,
                        help='Folder of the pretrained weights of high magnification, FOLDER < `simclr/runs/[FOLDER]`')
    parser.add_argument('--weights_low', default='weights_low', type=str,
                        help='Folder of the pretrained weights of low magnification, FOLDER <`simclr/runs/[FOLDER]`')
    parser.add_argument('--dataset', default='TCGA-lung', type=str,
    ...
    if args.magnification == 'tree' or args.magnification == 'low' or args.magnification == 'high':
        bags_path = os.path.join('/hdd/xb/WSI', args.dataset, 'pyramid', '*', '*')
    else:
        bags_path = os.path.join('/hdd/xb/WSI', args.dataset, 'single', '*', '*')
    feats_path = os.path.join('/hdd/yr/dataset/', args.dataset,'high')
    ...
    n_classes = glob.glob(os.path.join('/hdd/yr/dataset', args.dataset, '*' + os.path.sep))
    ...
    for i, item in enumerate(n_classes):
        ...
        bag_df['label'] = i
        bag_df.to_csv(os.path.join('/hdd/yr/dataset', args.dataset, item.split(os.path.sep)[2] + '.csv'),
                      index=False)
        all_df.append(bag_df)
    bags_path = pd.concat(all_df, axis=0, ignore_index=True)
    bags_path = shuffle(bags_path)
    bags_path.to_csv(os.path.join('/hdd/yr/dataset', args.dataset, args.dataset + '.csv'), index=False)
