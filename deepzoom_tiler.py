...
def nested_patches(img_slide, out_base, level=(0,), ext='jpeg'):
    ...
    img_class = img_slide.split(os.sep)[5]
    n_levels = len(glob.glob('/hdd/xb/WSI_temp_files/*'))
    bag_path = os.path.join(out_base, img_class, img_name)
    os.makedirs(bag_path, exist_ok=True)
    if len(level)==1:
        patches = glob.glob(os.path.join('/hdd/xb/WSI_temp_files', '*', '*.'+ext))
        ...
        levels = [int(os.path.basename(i)) for i in glob.glob(os.path.join('/hdd/xb/WSI_temp_files', '*'))]
        levels.sort()
        low_patches = glob.glob(os.path.join('/hdd/xb/WSI_temp_files', str(levels[0]), '*.'+ext))
        for i, low_patch in enumerate(low_patches):
            ...
            for x_pos in high_x_list:
                for y_pos in high_y_list:
                    high_patch = glob.glob(os.path.join('/hdd/xb/WSI_temp_files', str(levels[1]), '{}_{}.'.format(x_pos, y_pos)+ext))
                    if len(high_patch)!=0:
                        high_patch = high_patch[0]
                        shutil.move(high_patch, os.path.join(bag_path, low_patch_folder, high_patch.split(os.sep)[-1]))
                        high_patch_name = high_patch.split(os.sep)[-1]
                        high_patch_folder = high_patch_name.split('.')[0]
                        top_patch_path = os.path.join(bag_path, low_patch_folder, high_patch_folder)
                        os.makedirs(top_patch_path, exist_ok=True)
                        high_x = int(high_patch_folder.split('_')[0])
                        high_y = int(high_patch_folder.split('_')[1])
                        top_x_list = list( range(high_x*level_factor, (high_x+1)*level_factor) )
                        top_y_list = list( range(high_y*level_factor, (high_y+1)*level_factor) )
                        for x_pos_top in top_x_list:
                            for y_pos_top in top_y_list:
                                top_patch = glob.glob(os.path.join('/hdd/xb/WSI_temp_files', str(levels[2]), '{}_{}.'.format(x_pos_top, y_pos_top)+ext))
                                if len(top_patch)!=0:
                                    top_patch = top_patch[0]
                                    shutil.move(top_patch, os.path.join(bag_path, low_patch_folder, high_patch_folder, top_patch.split(os.sep)[-1]))
                        try:
                            os.rmdir(os.path.join(bag_path, low_patch_folder, high_patch_folder))
...
if __name__ == '__main__':
    ...
    assert len(levels)<=3, 'Only 1 or 2 or 3 magnifications are supported!'
    path_base = os.path.join('/hdd/xb/WSI', args.dataset)
    if len(levels) == 3:
        out_base = os.path.join('/hdd/xb/WSI', args.dataset, 'pyramid')
    else:
        out_base = os.path.join('/hdd/xb/WSI', args.dataset, 'single')
    ...
    for idx, c_slide in enumerate(all_slides):
        ...
        print(c_slide)
        DeepZoomStaticTiler(c_slide, '/hdd/xb/WSI_temp', levels, args.base_mag, args.objective, args.format, args.tile_size, args.overlap, True, args.quality, args.workers, args.background_t).run()
        nested_patches(c_slide, out_base, levels)
        shutil.rmtree('/hdd/xb/WSI_temp_files') 
